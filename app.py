...
with tab1:
    ...
    with st.expander("Connection Settings"):
        ...
        with col1:
            server = st.text_input("Server", value="192.168.10.2", disabled=True)
            selected_Db =st.selectbox("Select Database", list(database_selecttion.keys()))
            database = database_selecttion[selected_Db]
        with col2:
            username = st.text_input("Username", value="sa", disabled=True)
            password = st.text_input("Password", value="29031982", type="password",disabled=True)
